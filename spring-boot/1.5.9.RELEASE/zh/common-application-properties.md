## Appendix A.常见的应用程序属性

可以在 `application.properties` 文件内， `application.yml` 文件内或命令行开关中指定各种属性.本附录提供了常用Spring Boot属性的列表以及对使用它们的基础类的引用.

> Spring Boot提供具有高级值格式的各种转换机制，请务必查看[the properties conversion section](boot-features-external-config.html#boot-features-external-config-conversion).

> Property贡献可能来自类路径上的其他jar文件，因此您不应将此视为详尽的列表.此外，您可以定义自己的属性.

> T该样本文件仅供参考.将 **not** 复制并粘贴到您的应用程序中.相反，只选择您需要的属性.
